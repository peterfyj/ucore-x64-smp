...
char endian_buffer[SFS_BLKSIZE];
...
/* Add by Eternal Night */

#define htobe(buffer, origin, field, bits)		\
	(buffer)->field = htobe##bits ((origin)->field);

static void
inode_big_endian (struct inode *inode)
{
	struct inode *be_inode = (struct inode *)endian_buffer;
	int i;
	
	htobe (be_inode, inode, size, 32);
	htobe (be_inode, inode, type, 16);
	htobe (be_inode, inode, nlinks, 16);
	htobe (be_inode, inode, blocks, 32);
	for (i = 0; i < SFS_NDIRECT; i++) {
		htobe (be_inode, inode, direct[i], 32);
	}
	htobe (be_inode, inode, indirect, 32);
	htobe (be_inode, inode, db_indirect, 32);
}

static void
super_big_endian (struct super_block *super)
{
	struct super_block *be_super = (struct super_block *)endian_buffer;
	
	htobe (be_super, super, magic, 32);
	htobe (be_super, super, blocks, 32);
	htobe (be_super, super, unused_blocks, 32);
	memcpy (be_super->info, super->info, SFS_MAX_INFO_LEN + 1);
}

static void
entry_big_endian (struct sfs_entry *entry)
{
	struct sfs_entry *be_entry = (struct sfs_entry *)endian_buffer;

	htobe (be_entry, entry, ino, 32);
	memcpy (be_entry->name, entry->name, SFS_MAX_FNAME_LEN + 1);
}

static void
cache_block_big_endian (void *cache)
{
	uint32_t *be_block_entry = (uint32_t *)endian_buffer;
	uint32_t *original_entry = (uint32_t *)cache;
	
	int i;
	for (i = 0; i < SFS_BLKSIZE / sizeof (uint32_t); i++, be_block_entry++, original_entry ++)
		*be_block_entry = htobe32 (*original_entry);
}

/* Added contents end here */
...
flush_cache_block(struct sfs_fs *sfs, struct cache_block *cb) {
	...
	{
		cache_block_big_endian (cb->cache);
		//printf ("cache_block: ino = %d\n", cb->ino);
		write_block (sfs, endian_buffer, SFS_BLKSIZE, cb->ino);
	}
}
...
flush_cache_inode(struct sfs_fs *sfs, struct cache_inode *ci) {
	...
	{
		inode_big_endian (&(ci->inode));
		//printf ("cache_inode: ino = %d\n", ci->ino);
		write_block(sfs, endian_buffer, sizeof(ci->inode), ci->ino);
	}
}
...
close_sfs(struct sfs_fs *sfs) {
    ...
    for (i = 0; i < ninos; ino ++, i += SFS_BLKBITS) {
		...
		/* This block is the bitmap of the whole disk.
		 * As it is accessed by word, we have to transform it to big-endian.
		 */
		//printf ("close_sfs 1: ino = %d\n", ino);
        //write_block(sfs, buffer, sizeof(buffer), ino);
		{
			cache_block_big_endian (buffer);
			write_block(sfs, endian_buffer, sizeof(endian_buffer), ino);
		}
    }
    //write_block(sfs, &(sfs->super), sizeof(sfs->super), SFS_BLKN_SUPER);
	{
		super_big_endian (&(sfs->super));
		//printf ("close_sfs super: ino = %d\n", SFS_BLKN_SUPER);
		write_block(sfs, endian_buffer, sizeof(sfs->super), SFS_BLKN_SUPER);
	}
