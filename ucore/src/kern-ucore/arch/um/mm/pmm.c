 ...
 */
static int
get_pgtable_items_2 (size_t left, size_t right, size_t start, uintptr_t *pgdir, size_t *left_store, size_t *right_store)
{
	pte_t *pte;
	...
	while (start < right &&
		   ((pte = get_pte(pgdir, start * PGSIZE, 0)) == NULL  || !(*pte & PTE_P) ))
		start ++;
	if (start < right) {
		if (left_store != NULL)
			*left_store = start;
		...
		while (start < right &&
			   (pte = get_pte(pgdir, start * PGSIZE, 0)) != NULL && (*pte & PTE_USER) == perm) {
            start ++;
        }
        if (right_store != NULL) {
            *right_store = start;
