...
func Exit(code int) {
	Syscall(UCORE_EXIT, uintptr(code), 0, 0)
	return
}
...
func Gettimeofday(tv *Timeval) (errno int) {
	_, _, e1 := RawSyscall(UCORE_GETPCSTIME, uintptr(unsafe.Pointer(&tv.Sec)), uintptr(unsafe.Pointer(&tv.Usec)), 0)
	errno = int(e1)
	return
}
...
func Write(fd int, p []byte) (n int, errno int) {
	...
	r0, _, e1 := Syscall(UCORE_WRITE, uintptr(fd), uintptr(_p0), uintptr(len(p)))
	n = int(r0)
	errno = int(e1)
	return
}
...
func exitThread(code int) (errno int) {
	_, _, e1 := Syscall(UCORE_EXIT_THREAD, uintptr(code), 0, 0)
	errno = int(e1)
	return
}
...
func write(fd int, p *byte, np int) (n int, errno int) {
	r0, _, e1 := Syscall(UCORE_WRITE, uintptr(fd), uintptr(unsafe.Pointer(p)), uintptr(np))
	n = int(r0)
	errno = int(e1)
	return
}
